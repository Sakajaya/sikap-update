...
if (!function_exists('can_access_cbt_test_status')) {
    ...
    function can_access_cbt_test_status(int $testStatusId): bool
    {
        ...
        // Guru bisa akses jika:
        // 1. Ia yang membuat jadwal ujian (created_by), ATAU
        // 2. Bank soal ujian tersebut miliknya, ATAU
        // 3. Salah satu kelas yang diampu masuk dalam class_codes ujian
        if ($context['is_teacher'] && $context['teacher_id'] && $context['user_id']) {
            $db = \Config\Database::connect();

            $testStatus = $db->table('cbt_test_status ts')
                ->select('ts.created_by, ts.class_codes, qb.teacher_id AS bank_teacher_id')
                ->join('cbt_question_banks qb', 'qb.id = ts.bank_id', 'left')
                ->where('ts.id', $testStatusId)
                ->get()
                ->getRowArray();

            if (!$testStatus) return false;

            // Cek 1: pembuat jadwal
            if ((int)$testStatus['created_by'] === $context['user_id']) return true;

            // Cek 2: bank soal milik guru ini
            if ((int)$testStatus['bank_teacher_id'] === $context['teacher_id']) return true;

            // Cek 3: salah satu kelas yang diampu ada di class_codes
            $classCodes = json_decode($testStatus['class_codes'] ?? '[]', true);
            if (!empty($classCodes)) {
                ...
                foreach ($classCodes as $code) {
                    if (in_array(strtolower(trim((string)$code)), $teacherClassNames)) {
                        return true;
                    }
                }
            }
...
if (!function_exists('get_teacher_classes')) {
    ...
    function get_teacher_classes(int $teacherId): array
    {
        ...
        // Ambil juga kelas di mana guru tersebut menjadi Wali Kelas
        $waliClasses = $db->table('classes c')
            ->select('c.id, c.name, c.level')
        ...
        foreach ($waliClasses as $wc) {
            if (!in_array($wc['id'], $addedIds)) {
                $result[] = $wc;
                $addedIds[] = $wc['id'];
            }
        }
...
if (!function_exists('update_last_activity_throttled')) {
    /**
     * Update last_activity on cbt_sessions with throttling.
     * Only writes to DB if the last update was more than $intervalSeconds ago,
     * to avoid hammering the database on every keystroke.
     *
     * @param int $studentId
     * @param int $testId
     * @param int $intervalSeconds  Minimum seconds between DB writes (default: 30)
     */
    function update_last_activity_throttled(int $studentId, int $testId, int $intervalSeconds = 30): void
    {
        try {
            $db = \Config\Database::connect();

            // Read current last_activity to decide whether to update
            ...
            $lastActivity = $session['last_activity'] ? strtotime($session['last_activity']) : 0;
            $now = time();

            if (($now - $lastActivity) >= $intervalSeconds) {
                $db->table('cbt_sessions')
                    ->where('student_id', $studentId)
                    ->where('test_id', $testId)
                    ->set(['last_activity' => date('Y-m-d H:i:s', $now)])
                    ->update();

                log_message('debug', "[update_last_activity_throttled] Updated for student=$studentId, test=$testId");
            }
        } catch (\Throwable $e) {
            log_message('error', '[update_last_activity_throttled] ' . $e->getMessage());
        }
    }
}
