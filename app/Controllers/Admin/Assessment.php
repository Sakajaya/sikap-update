...
class Assessment extends BaseController
{
    ...
    public function formatifList($classId, $subjectId)
    {
        ...
        // Cek apakah mapel ini diampu oleh lebih dari 1 guru di level yang sama.
        // Jika single-guru: ATP disimpan di salah satu kelas se-level → ambil semua se-level.
        // Jika multi-guru: tiap guru punya ATP per kelas sendiri → filter spesifik per class_id.
        ...
        $atpBuilder = $this->db->table('alur_tujuan_pembelajaran atp')
        ...
        if ($isMultiGuru) {
            // Multi-guru: filter ketat per kelas
            $atpBuilder->where('atp.class_id', $classId);
        } else {
            // Single-guru: ambil semua ATP se-level (ATP mungkin tersimpan di kelas lain)
            $atpBuilder->where('c_atp.level', $classLevel);
        }

        $materials = $atpBuilder
            ->groupBy('atp.id')
    ...
    public function input($classId, $subjectId, $type)
    {
        ...
        // Default: semua siswa kelas (filter tahun ajaran aktif)
        ...
        if ($type === 'formatif') {
            // Ambil materi dari ATP — single-guru: ambil se-level, multi-guru: per kelas
            ...
            $matBuilder = $this->db->table('alur_tujuan_pembelajaran atp')
                ->select('atp.id, atp.lingkup_materi as title')
                ->join('classes c_atp', 'c_atp.id = atp.class_id', 'left')
                ->where('atp.subject_id', $subjectId);
    ...
    public function store()
    {
        ...
        // ── Notifikasi nilai masuk: kirim ke setiap siswa yang dinilai ───────
        try {
            // Ambil nama mapel
            $subjectId   = $data['subject_id'] ?? null;
            $subjectName = $subjectId
                ? ($this->db->table('subjects')->select('name')->where('id', $subjectId)->get()->getRowArray()['name'] ?? 'Mata Pelajaran')
                : 'Mata Pelajaran';

            // Tentukan label tipe penilaian
            ...
            // Kumpulkan user_id dari student_id yang punya nilai (tidak kosong)
            $studentIds = [];
            foreach ($data['scores'] as $studentId => $score) {
                if ($score !== '' && $score !== null) {
            ...
            if (!empty($studentIds)) {
                // Resolusi user_id dari student_id via tabel students → users
                $userRows = $this->db->table('students s')
                ...
                if (!empty($userIds)) {
                    notify_users(
                        $userIds,
                        'nilai_masuk',
                        "Nilai {$typeLabel} Masuk: {$subjectName}",
                        "Guru telah memasukkan nilai {$typeLabel} untuk mata pelajaran {$subjectName}.",
                        base_url('siswa/grades')
                    );
                }
            }
        } catch (\Throwable $e) {
            log_message('warning', '[Assessment::store] Notifikasi gagal: ' . $e->getMessage());
        }
        // ─────────────────────────────────────────────────────────────────────
