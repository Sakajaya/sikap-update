...
class CbtTestStatus extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($context['is_admin']) {
            ...
            $classes = $this->classModel->getClassesByAcademicYear((int)($activeYear['id'] ?? 0));
        } elseif ($context['is_teacher'] && $context['teacher_id']) {
    ...
    public function store()
    {
        ...
        // -------------------------------------------------------
        // 2. Ambil data bank soal untuk validasi jumlah soal
        // -------------------------------------------------------
        $bankId = $post['bank_id'] ?? 0;

        // Ambil data bank beserta hitungan soal real-time
        $allBanks = $this->bankModel->getListWithCounts();
        $bank = null;
