    ...
    <div class="card-body">
      <form action="<?= base_url('admin/classes/store') ?>" method="post">
        ...
        <div class="mb-3">
          ...
          <input type="text" name="name" class="form-control" placeholder="Contoh: 7A, 7B" value="<?= old('name') ?>" required>
          <small class="text-muted">Masukkan nama kelas (misal: 7A untuk kelas 7 rombel A)</small>
        </div>
        
        <div class="mb-3">
          <label class="form-label">Tingkat <span class="text-danger">*</span></label>
          <input type="number" name="level" class="form-control" min="<?= $minLevel ?>" max="<?= $maxLevel ?>" 
                 placeholder="<?= $minLevel ?> - <?= $maxLevel ?>" value="<?= old('level') ?>" required>
