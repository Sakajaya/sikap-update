...
<head>
    <meta charset="UTF-8">
    <title>Dokumen Rencana Kokurikuler - Lintas Disiplin Ilmu</title>
    <style>
        body {
...
<body>
    ...
    <div class="header">
        ...
        <h3>KOLABORATIF LINTAS DISIPLIN ILMU</h3>
    ...
    <div class="section">
        ...
        <table class="info-table">
            ...
            <tr>
                ...
                <td>Kolaboratif Lintas Disiplin Ilmu</td>
            </tr>
        </table>
    </div>
    ...
    <div class="section">
        <div class="section-title">C. MATA PELAJARAN YANG TERLIBAT</div>
        <?php 
        ...
        ?>
            <p>Kegiatan ini melibatkan kolaborasi lintas disiplin ilmu dari beberapa mata pelajaran untuk mencapai pembelajaran yang holistik dan bermakna.</p>
            
            <?php $no = 1; foreach ($groupedBySubject as $subjectId => $items): ?>
                <div style="margin-bottom: 15px;">
                    <strong><?= $no++ ?>. <?= isset($subjectNames[$subjectId]) ? esc($subjectNames[$subjectId]) : 'Mata Pelajaran ID: ' . $subjectId ?></strong>
                    ...
                    <?php if (!empty($items)): ?>
                        <ul style="margin-top: 5px; margin-bottom: 0;">
                            <?php foreach ($items as $item): ?>
                                ...
                                // Find TP data
                                $tpData = null;
                                if (isset($subjectTPs[$subjectId])) {
                                    foreach ($subjectTPs[$subjectId] as $tp) {
                                        if ($tp['id'] == $tpId) {
                                            $tpData = $tp;
                                            break;
                                        }
                                ...
                                <?php if ($tpData): ?>
                                    <li style="margin-bottom: 3px;">
                                        <?php if (!empty($tpData['kode_tp'])): ?>
                                            <strong><?= esc($tpData['kode_tp']) ?>.</strong> 
                                        <?php endif; ?>
                                        <?= esc($tpData['deskripsi']) ?>
                                        <br><em style="color: #666; font-size: 0.9em;">
                                            Dimensi: <?= esc($dimensi) ?>
                                            <?php if (!empty($tpData['sub_dimensi'])): ?>
                                                | Sub Dimensi: <?= esc($tpData['sub_dimensi']) ?>
                                            <?php endif; ?>
                                        </em>
                                    </li>
                                <?php endif; ?>
