...
<div class="container">
  <h3>Tugas dan Agenda</h3>

  <!-- 🔹 Navigasi bulan & tahun -->
  <div class="row g-2 mb-3 align-items-center">
    <div class="col-auto">
      <a href="<?= site_url("admin/agendas/$prevYear/" . str_pad($prevMonth, 2, '0', STR_PAD_LEFT)) ?>"
    ...
    <div class="col-auto">
      <a href="<?= site_url("admin/agendas/$nextYear/" . str_pad($nextMonth, 2, '0', STR_PAD_LEFT)) ?>"
        class="btn btn-outline-secondary btn-sm">▶</a>
    </div>
  </div>
  ...
  <div class="mb-3 d-flex align-items-center gap-3">
    <div class="d-flex align-items-center">
      <div
        style="width:18px;height:18px;background:#f8d7da;border-radius:3px;border:1px solid rgba(0,0,0,0.08);margin-right:6px">
      </div>
    ...
    <div class="d-flex align-items-center">
      ...
      <small class="text-muted">Sabtu & Minggu</small>
    </div>
          ...
          <?php foreach ($w as $i => $d): ?>
            ...
            <?php else:
              ...
              if ($isHoliday || $isWeekend) {
                $classes[] = 'bg-danger';
                $classes[] = 'bg-opacity-10'; // gunakan opacity ringan
              }
              ?>
              <td class="<?= implode(' ', $classes) ?>" style="height:80px; cursor:pointer;" data-date="<?= $date ?>">
  ...
  <a href="<?= site_url('admin/agendas/create') ?>" class="btn btn-primary btn-sm">Buat Agenda</a>
</div>
...
<script>
  $(function () {
    // Prevent AJAX caching for all subsequent requests
    ...
    $('#monthSelect, #yearSelect').on('change', function () {
      ...
      window.location.href = '<?= site_url('admin/agendas') ?>/' + y + '/' + m;
    });
    ...
    $(document).on('click', '.calendar-day', function () {
      ...
      // jika klik tanggal yang sama saat baris terbuka -> tutup
      if (weekAgendaRow.is(':visible') && container.data('date') === date) {
        weekAgendaRow.hide();
        $('.calendar-day').removeClass('bg-primary text-white'); // reset highlight
        return;
      }

      // tutup baris lain & reset highlight
      $('tr.week-agenda-row').hide();
      $('.calendar-day').removeClass('bg-primary text-white');

      $.get('<?= site_url('admin/agendas/date') ?>/' + date, function (html) {
        container.html(html);
        container.data('date', date);
        weekAgendaRow.show();
        // beri highlight biru penuh pada tanggal aktif
