...
<?php foreach ($messages as $msg):
  ...
  if ($lastDate !== $msgDate) {
    ...
    if ($msgDate === $today) {
      ...
      $displayDate = date('d F Y', strtotime($msgDate));
    }
    $lastDate = $msgDate;
  }
  ?>
  ...
  <div id="msg-<?= $msg['id'] ?>"
    class="chat-message <?= $msg['user_id'] == session('user.id') ? 'text-end' : 'text-start' ?>">
    ...
    <div class="chat-bubble <?= $msg['user_id'] == session('user.id') ? 'sent' : 'received' ?>">
      ...
      <?php if (!empty($msg['attachment'])): ?>
        <div class="chat-attachment mb-2">
          <a href="<?= base_url('uploads/chat/' . $msg['attachment']) ?>" class="chat-img-link">
            <img src="<?= base_url('uploads/chat/' . $msg['attachment']) ?>" class="img-fluid rounded"
              style="max-width: 200px; max-height: 200px; object-fit: cover;">
          </a>
        </div>
      <?php endif; ?>

      <!-- Isi pesan (dengan mention diwarnai) -->
      ...
      <div class="chat-footer">
        ...
        <a href="javascript:void(0)" class="chat-action btn-reply" data-id="<?= $msg['id'] ?>"
